...
def piper_dataset_transform(trajectory: Dict[str, Any]) -> Dict[str, Any]:
    ...
    gripper = tf.clip_by_value(trajectory["action"][:, -1:], 0.0, 1.0)
    trajectory["action"] = tf.concat([trajectory["action"][:, :-1], gripper], axis=1)
    return trajectory
...
OXE_STANDARDIZATION_TRANSFORMS = {
    ...
    "pick_up_the_banana": piper_dataset_transform,
}

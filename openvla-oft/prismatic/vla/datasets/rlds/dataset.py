...
def _builder_from_prepared_or_registry(name, data_dir):
    ...
    for root in candidates:
        if (root / "dataset_info.json").exists():
            print(f"[TFDS] Loading prepared dataset from {root}")
            return tfds.builder_from_directory(builder_dir=str(root))
        ...
        if root.exists():
            ...
            if version_dirs:
                ...
                print(f"[TFDS] Loading prepared dataset from {version_dir}")
                return tfds.builder_from_directory(builder_dir=str(version_dir))

    print(f"[TFDS] Falling back to registry builder: {name}")
    return tfds.builder(name, data_dir=str(data_dir))
# ruff: noqa: B006
def make_dataset_from_rlds(
    ...
    *,
    ...
    builder = _builder_from_prepared_or_registry(name, data_dir)
